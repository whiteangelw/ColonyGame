...
public class SaveGameService : MonoBehaviour
{
    ...
    private SaveGameData CaptureGame()
    {
        ...
        CaptureCreatures(data);
        CaptureCreatureKnowledge(data);
        CaptureFlora(data);
    ...
    private static void CaptureWorld(SaveGameData data, GridManager grid)
    {
        ...
        data.world.structureTileTypes = new int[cellCount];
        ...
        for (int x = 0; x < grid.width; x++)
        {
            for (int y = 0; y < grid.height; y++)
            {
                ...
                data.world.structureTileTypes[cellIndex] =
                    (int)tile.structureType;
    ...
    private static void CaptureStructures(SaveGameData data)
    {
        ...
        foreach (ConfiguredStructure structure in manager.GetRegisteredConfiguredStructures())
        {
            ...
            PlanterStructureBehaviour planter =
                structure.GetComponent<PlanterStructureBehaviour>();
            ProductionMachineBehaviour productionMachine =
                structure.GetComponent<ProductionMachineBehaviour>();
            StructureSaveData savedConfigured = new StructureSaveData
            {
                ...
                hasPlanterState = planter != null,
                planterState = planter != null
                    ? (int)planter.State
                    : 0,
                planterRemainingGrowthTime = planter != null
                    ? planter.RemainingGrowthTime
                    : 0f,
                planterCropId = planter != null
                    ? planter.SelectedCropId
                    : string.Empty,
                hasProductionState = productionMachine != null,
                productionRecipeId = productionMachine != null
                    && productionMachine.ActiveRecipe != null
                        ? productionMachine.ActiveRecipe.RecipeId
                        : string.Empty,
                productionRemainingBatches = productionMachine != null
                    ? productionMachine.RemainingBatches
                    : 0,
                productionCompletedWork = productionMachine != null
                    ? productionMachine.CompletedWork
                    : 0f
            };

            if (planter != null)
            {
                AppendResourceAmounts(
                    savedConfigured.planterDeliveredMaterials,
                    planter.GetDeliveredMaterialsSnapshot());
            }

            if (productionMachine != null)
            {
                AppendResourceAmounts(
                    savedConfigured.productionInputs,
                    productionMachine.GetInputBufferSnapshot());
                AppendResourceAmounts(
                    savedConfigured.productionOutputs,
                    productionMachine.GetOutputBufferSnapshot());
            }

            data.structures.Add(savedConfigured);
        }
    }

    private static void AppendResourceAmounts(
        List<ResourceAmountSaveData> destination,
        Dictionary<ResourceType, int> source)
    {
        foreach (KeyValuePair<ResourceType, int> entry in source)
        {
            if (entry.Value <= 0) continue;
            destination.Add(new ResourceAmountSaveData
            {
                resourceType = (int)entry.Key,
                amount = entry.Value
            });
    ...
    private static void CaptureCreatures(SaveGameData data)
    {
        if (data == null) return;
        data.hasCreatureState = true;

        IReadOnlyList<CreatureController> creatures = CreatureRegistry.Creatures;
        for (int i = 0; i < creatures.Count; i++)
        {
            ...
            creature.RefreshGridPosition();
            data.creatures.Add(new CreatureSaveData
            {
                creatureId = creature.Definition.creatureId,
                gridX = creature.GridPosition.x,
                gridY = creature.GridPosition.y,
                homeX = creature.HomePosition.x,
                homeY = creature.HomePosition.y,
                hasNaturalHome = true,
                naturalHomeX = creature.NaturalHomePosition.x,
                naturalHomeY = creature.NaturalHomePosition.y,
                spawnOrigin = creature.SpawnOrigin,
                biomeId = creature.SpawnBiomeId,
                hasHabitat = creature.HabitatLink != null
                    && creature.HabitatLink.HasAssignedHabitat,
                habitatX = creature.HabitatLink != null
                    ? creature.HabitatLink.HabitatGridPosition.x : 0,
                habitatY = creature.HabitatLink != null
                    ? creature.HabitatLink.HabitatGridPosition.y : 0,
                domestication = creature.Domestication != null
                    ? creature.Domestication.CaptureState()
                    : new CreatureDomesticationSaveData()
            });
        }
    }

    private static void CaptureCreatureKnowledge(SaveGameData data)
    {
        if (data == null) return;
        CreatureKnowledgeService service = CreatureKnowledgeService.Instance;
        data.creatureKnowledge = service != null
            ? service.CaptureState()
            : new CreatureKnowledgeSaveData();
    }
    ...
    private static void CapturePersistentTasks(SaveGameData data)
    {
        ...
        foreach (Task task in TaskManager.Instance.GetTasksSnapshot())
        {
            ...
            data.persistentTasks.Add(new TaskSaveData
            {
                ...
                workCompleted = task.workCompleted,
                preserveWorkOnInterruption = task.preserveWorkOnInterruption
            });
        }
    }
            ...
            try
            {
                using (RestoreMarker.Auto())
                {
                    ...
                    using (RestoreEntitiesMarker.Auto())
                    {
                        ...
                        RestoreCreatures(data.creatures);
                ...
                using (PostRestoreMarker.Auto())
                {
                    ...
                    RestoreCreatureKnowledge(data.creatureKnowledge);
    ...
    private static void ClearRuntimeWorld()
    {
        List<CreatureController> creatures =
            new List<CreatureController>(CreatureRegistry.Creatures);
        for (int i = 0; i < creatures.Count; i++)
        {
            CreatureController creature = creatures[i];
        ...
        TileSaveData tile)
    {
        grid.RestoreTileState(
            ...
            (TileType)tile.structureTileType,
    ...
    private static TileType ResolveStructureTileType(
        WorldSaveData world,
        int cellIndex,
        string structureContentId)
    {
        if (world.structureTileTypes != null
            && cellIndex >= 0
            && cellIndex < world.structureTileTypes.Length)
        {
            return (TileType)world.structureTileTypes[cellIndex];
        }

        if (string.IsNullOrWhiteSpace(structureContentId))
        {
            return TileType.Empty;
        }

        BuildDefinitionSO definition =
            BuildCatalogService.Instance?.GetById(structureContentId);
        ...
        List<StructureSaveData> structures)
    {
        ...
        foreach (StructureSaveData savedStructure in structures)
        {
            ...
            RestorePlanterState(position, savedStructure);
            RestoreProductionState(position, savedStructure);
    ...
    private static void RestorePlanterState(
        Vector2Int position,
        StructureSaveData savedStructure)
    {
        if (!savedStructure.hasPlanterState
            || StructureManager.Instance == null)
        {
            return;
        }

        foreach (ConfiguredStructure configured in
                 StructureManager.Instance.GetRegisteredConfiguredStructures())
        {
            if (configured == null || configured.GridPosition != position)
            {
                continue;
            }

            PlanterStructureBehaviour planter =
                configured.GetComponent<PlanterStructureBehaviour>();
            planter?.RestoreState(
                (PlanterState)savedStructure.planterState,
                savedStructure.planterRemainingGrowthTime,
                savedStructure.planterCropId,
                savedStructure.planterDeliveredMaterials);
            return;
        }
    }

    private static void RestoreProductionState(
        Vector2Int position,
        StructureSaveData savedStructure)
    {
        if (!savedStructure.hasProductionState
            || StructureManager.Instance == null)
        {
            return;
        }
        ...
        foreach (ConfiguredStructure configured in
                 StructureManager.Instance.GetRegisteredConfiguredStructures())
        {
            if (configured == null || configured.GridPosition != position) continue;
            ProductionMachineBehaviour machine =
                configured.GetComponent<ProductionMachineBehaviour>();
            machine?.RestoreState(
                savedStructure.productionRecipeId,
                savedStructure.productionRemainingBatches,
                savedStructure.productionCompletedWork,
                savedStructure.productionInputs,
                savedStructure.productionOutputs);
            return;
        }
    }
    ...
    private static void RestoreCreatures(List<CreatureSaveData> creatures)
    {
        if (creatures == null || creatures.Count == 0) return;

        CreatureSpawnService spawnService = CreatureSpawnService.Instance;
        if (spawnService == null)
        {
            Debug.LogWarning(
                "[SaveGameService] CreatureSpawnService não encontrado; "
                + "criaturas não foram restauradas.");
            return;
        }

        for (int i = 0; i < creatures.Count; i++)
        {
            CreatureSaveData saved = creatures[i];
            if (saved == null || string.IsNullOrWhiteSpace(saved.creatureId))
                continue;

            CreatureController creature = spawnService.SpawnRestored(
                saved.creatureId,
                new Vector2Int(saved.gridX, saved.gridY),
                new Vector2Int(saved.homeX, saved.homeY),
                saved.spawnOrigin,
                saved.biomeId);
            if (creature == null) continue;

            Vector2Int naturalHome = saved.hasNaturalHome
                ? new Vector2Int(saved.naturalHomeX, saved.naturalHomeY)
                : saved.hasHabitat
                    ? new Vector2Int(saved.gridX, saved.gridY)
                    : new Vector2Int(saved.homeX, saved.homeY);
            creature.SetNaturalHomePosition(naturalHome);

            creature.Domestication?.RestoreState(saved.domestication);
            if (saved.hasHabitat)
            {
                creature.HabitatLink?.RestoreAssignment(
                    new Vector2Int(saved.habitatX, saved.habitatY));
            }
            creature.Brain?.ForceDecision();
        }
    }
                ...
                || restoredType == TaskType.Chop)
            {
                ...
                if (flora != null)
                {
                    TaskManager.Instance.AddHarvestTask(
                        flora,
                        savedTask.priority);
                    Task restoredHarvest = TaskManager.Instance.GetTaskAt(
                        restoredPosition);
                    if (restoredHarvest != null && savedTask.workRequired > 0f)
                    {
                        restoredHarvest.RestoreWork(
                            savedTask.workRequired,
                            savedTask.workCompleted,
                            savedTask.preserveWorkOnInterruption);
                    }
                }
                continue;
            }
    ...
    private static void RestoreCreatureKnowledge(
        CreatureKnowledgeSaveData savedState)
    {
        CreatureKnowledgeService service = CreatureKnowledgeService.Instance;
        if (service == null) return;

        // Saves anteriores não possuíam conhecimento de criaturas.
        if (savedState == null || !savedState.hasState)
        {
            service.ResetKnowledge();
            return;
        }
