...
public sealed class DuplicantGroundHaulExecutor
{
    ...
    private readonly DuplicantCarriedResourceRouter resourceRouter;
    ...
    public DuplicantGroundHaulExecutor(
        ...
        DuplicantCarriedResourceRouter resourceRouter,
        ...
        List<Vector2Int> initialPath)
    {
        ...
        {
            yield return resourceRouter.TryDeliverToDemandTargets(task);

            if (!inventory.HasItem)
            {
                storageReservation.Release();
                CompleteGroundHaulTask(task, item);
    ...
    private static void CompleteGroundHaulTask(
        Task task,
        ResourceItem item)
    {
