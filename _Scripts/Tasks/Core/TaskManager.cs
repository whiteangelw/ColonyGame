...
public class TaskManager : MonoBehaviour
{
    ...
    private readonly List<Task> batchCandidateBuffer =
        new List<Task>(64);
        ...
        GridLayer targetLayer = GridLayer.Terrain)
    {
        ...
        int categoryPriority = PriorityManager.Instance != null
            ? PriorityManager.Instance.GetCategoryPriority(type)
            : 5;
    ...
    public void AddTask(Task task)
    {
        ...
        if (pendingTasks.Exists(existing => IsSameTask(existing, task)))
        {
            return;
        }
        ...
        TileType tileToBuild)
    {
        ...
        int categoryPriority = PriorityManager.Instance != null
            ? PriorityManager.Instance.GetCategoryPriority(
                TaskType.BuildTile)
            : 5;
    ...
    public void AddHaulTask(ResourceItem item)
    {
        ...
        bool alreadyExists = pendingTasks.Exists(
            task => task.type == TaskType.HaulResource
    ...
    public Task GetTaskAt(Vector2Int gridPos)
    {
        for (int i = 0; i < pendingTasks.Count; i++)
        {
            ...
            if (task != null && task.gridPosition == gridPos)
            {
                return task;
            }
        }
        ...
        out Task selectedTask)
    {
        ...
        int maximumDistance = MaximumBlueprintDeliveryDistance;
        ...
        candidateSortOrigin = batchAnchor;
        batchCandidateBuffer.Sort(CompareBlueprintBatchCandidate);
    ...
    private int CompareBlueprintBatchCandidate(Task a, Task b)
    {
    ...
    public bool IsTaskValid(Task task)
    {
        ...
        switch (task.type)
        {
            ...
            case TaskType.Dismantle:
                return WorldInteractionService.Instance != null
                    && WorldInteractionService.Instance
                        .CanDismantleAt(
                            task.gridPosition,
                            task.targetLayer);
    ...
    private sealed class TaskCandidateComparer
        : IComparer<TaskCandidate>
    {
