...
public readonly struct PerformanceMetricsSnapshot
{
    ...
    public readonly float NavPartialAverageMs;
    public readonly float NavPartialMaximumMs;
    public readonly int NavRegionsPendingPeak;
    public readonly int NavForcedFlushes;
    ...
    public PerformanceMetricsSnapshot(
        ...
        int pendingTasks,
        int activeItems,
        int activeDuplicants,
        float brainTickRate,
        float needsTickRate,
        int fullNavRegenerations,
        int partialNavRegenerations,
        int navCellsRegenerated,
        float navPartialAverageMs,
        float navPartialMaximumMs,
        int navRegionsPendingPeak,
        int navForcedFlushes,
        int globalObjectSearches,
        long gcAllocatedBytesPerFrame,
        float lastSaveMs,
        float lastLoadMs,
        float liquidVisualizerAverageMs,
        float liquidVisualizerMaximumMs,
        int liquidCellsProcessed,
        int liquidDirtyCellsPending,
        int liquidDepthColumnsPending,
        float liquidSimulationAverageMs,
        ...
        float lastSaveMenuMetadataMs,
        float lastSaveReadFileMs,
        float lastSaveDeserializeMs,
        float lastSaveCaptureMs,
        float lastSaveSerializeMs,
        float lastSaveWriteMs,
        float lastLoadClearWorldMs,
        float lastLoadRestoreGridMs,
        float lastLoadRestoreEntitiesMs,
        float lastLoadPostRestoreMs,
        float lastWorldGenerationMs,
        float lastTilemapFullRefreshMs,
        float tilemapRegionalAverageMs,
        ...
        int fogRegionalRegionsPendingPeak)
    {
        ...
        NavPartialAverageMs = navPartialAverageMs;
        NavPartialMaximumMs = navPartialMaximumMs;
        NavRegionsPendingPeak = navRegionsPendingPeak;
        NavForcedFlushes = navForcedFlushes;
...
public sealed class PerformanceMetricsService : MonoBehaviour
{
    ...
    private float samplingInterval = 1f;
    ...
    private int navPartialSamples;
    private double navPartialTotalMs;
    private double navPartialMaximumMs;
    private int navRegionsPendingPeak;
    private int navForcedFlushes;
    ...
    private void Update()
    {
        ...
        Snapshot = new PerformanceMetricsSnapshot(
            ...
            TaskManager.Instance != null
                ? TaskManager.Instance.PendingTaskCount
                : 0,
            activeResourceItems,
            activeDuplicants,
            brainTicks / elapsed,
            ...
            navCellsRegenerated,
            Average(navPartialTotalMs, navPartialSamples),
            (float)navPartialMaximumMs,
            navRegionsPendingPeak,
            navForcedFlushes,
            ...
            lastSaveReadFileMs,
            lastSaveDeserializeMs,
            lastSaveCaptureMs,
            lastSaveSerializeMs,
            lastSaveWriteMs,
            lastLoadClearWorldMs,
            lastLoadRestoreGridMs,
            lastLoadRestoreEntitiesMs,
            lastLoadPostRestoreMs,
            lastWorldGenerationMs,
            lastTilemapFullRefreshMs,
            Average(
                tilemapRegionalTotalMs,
        ...
        fullNavRegenerations = 0;
        partialNavRegenerations = 0;
        navCellsRegenerated = 0;
        navPartialSamples = 0;
        navRegionsPendingPeak = 0;
        navForcedFlushes = 0;

        globalObjectSearches = 0;

        liquidVisualizerSamples = 0;
        liquidCellsProcessed = 0;

        liquidSimulationSamples = 0;
        liquidBufferResizes = 0;
        liquidSimulationCells = 0L;

        tilemapRegionalSamples = 0;
        tilemapRegionalCellsProcessed = 0;
        tilemapRegionalCellsPendingPeak = 0;
        tilemapRegionalRegionsPendingPeak = 0;

        fogRegionalSamples = 0;
        fogRegionalCellsProcessed = 0;
        ...
        liquidVisualizerTotalMs = 0d;
        liquidVisualizerMaximumMs = 0d;

        liquidSimulationTotalMs = 0d;
        liquidSimulationMaximumMs = 0d;

        tilemapRegionalTotalMs = 0d;
        tilemapRegionalMaximumMs = 0d;

        fogRegionalTotalMs = 0d;
        fogRegionalMaximumMs = 0d;

        navPartialTotalMs = 0d;
        navPartialMaximumMs = 0d;
        ...
        double milliseconds)
    {
        switch (metric)
        {
            ...
            case PerformanceMetric.NavGraphPartial:
                navPartialSamples++;
                navPartialTotalMs += milliseconds;
                navPartialMaximumMs = Math.Max(
                    navPartialMaximumMs,
                    milliseconds);
                break;
            ...
            case PerformanceMetric.SaveReadFile:
                lastSaveReadFileMs = (float)milliseconds;
                break;

            case PerformanceMetric.SaveDeserialize:
                lastSaveDeserializeMs = (float)milliseconds;
                break;

            case PerformanceMetric.SaveCapture:
                lastSaveCaptureMs = (float)milliseconds;
                break;

            case PerformanceMetric.SaveSerialize:
                lastSaveSerializeMs = (float)milliseconds;
                break;

            case PerformanceMetric.SaveWrite:
                lastSaveWriteMs = (float)milliseconds;
                break;

            case PerformanceMetric.LoadClearWorld:
                lastLoadClearWorldMs = (float)milliseconds;
                break;

            case PerformanceMetric.LoadRestoreGrid:
                lastLoadRestoreGridMs = (float)milliseconds;
                break;

            case PerformanceMetric.LoadRestoreEntities:
                lastLoadRestoreEntitiesMs = (float)milliseconds;
                break;

            case PerformanceMetric.LoadPostRestore:
                lastLoadPostRestoreMs = (float)milliseconds;
                break;

            case PerformanceMetric.WorldGeneration:
                lastWorldGenerationMs = (float)milliseconds;
                break;

            case PerformanceMetric.TilemapFullRefresh:
                lastTilemapFullRefreshMs = (float)milliseconds;
                break;
            ...
            case PerformanceMetric.FogFullRefresh:
                lastFogFullRefreshMs = (float)milliseconds;
                break;
        ...
        double milliseconds)
    {
        ...
        Instance.AddDuration(metric, Math.Max(0d, milliseconds));
#endif
    }
        ...
        int pendingDepthColumns)
    {
        ...
        Instance.liquidDirtyCellsPending = Mathf.Max(
            0,
            pendingCells);
        ...
        int pendingRegions)
    {
        ...
        Instance.tilemapRegionalCellsProcessed += Mathf.Max(
            0,
            processedCells);
        ...
        int pendingRegions)
    {
        ...
        Instance.fogRegionalCellsProcessed += Mathf.Max(
            0,
            processedCells);
    ...
    public static void RecordNavRegeneration(bool full, int cells)
    {
        ...
        if (full)
        {
            Instance.fullNavRegenerations++;
        }
        else
        {
            Instance.partialNavRegenerations++;
        }
    ...
    public static void RecordNavRegionalQueue(
        int pendingRegions,
        bool forcedFlush)
    {
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        ...
        Instance.navRegionsPendingPeak = Mathf.Max(
            Instance.navRegionsPendingPeak,
            pendingRegions);

        if (forcedFlush)
        {
            Instance.navForcedFlushes++;
        }
#endif
    }
    ...
    public static void ChangeActiveResourceItems(int delta)
    {
        ...
        if (Instance != null)
        {
            Instance.activeResourceItems = Mathf.Max(
                0,
                Instance.activeResourceItems + delta);
        }
#endif
    }
    ...
    public static void ChangeActiveDuplicants(int delta)
    {
        ...
        if (Instance != null)
        {
            Instance.activeDuplicants = Mathf.Max(
                0,
                Instance.activeDuplicants + delta);
        }
#endif
    }
}