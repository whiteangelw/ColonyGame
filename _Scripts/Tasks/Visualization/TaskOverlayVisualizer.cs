...
public class TaskOverlayVisualizer : MonoBehaviour
{
    ...
    private void OnTaskAddedHandler(Task task)
    {
        ...
        Vector2Int visualPosition = task.visualGridPosition;
        Vector3Int tilePosition = new Vector3Int(visualPosition.x, visualPosition.y, 0);
        ...
        if (priorityNumberPrefab != null && !activePriorityTexts.ContainsKey(visualPosition))
        {
            ...
            activePriorityTexts.Add(visualPosition, textObj);
        }
    }
    ...
    private void OnTaskRemovedHandler(Task task)
    {
        ...
        Task remainingTask = TaskManager.Instance?
            .GetTasksSnapshot()
            .Find(candidate => candidate != null
                && candidate.visualGridPosition == visualPosition);

        if (remainingTask != null)
        {
            UpdateTaskPriorityText(
                visualPosition,
                remainingTask.priority);
            return;
        }
        ...
        if (activePriorityTexts.TryGetValue(visualPosition, out GameObject textObj))
        {
            if (textObj != null)
            {
            ...
            activePriorityTexts.Remove(visualPosition);
        }
    }
