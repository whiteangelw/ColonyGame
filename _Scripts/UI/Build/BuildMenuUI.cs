...
public class BuildMenuUI : MonoBehaviour
{
    ...
    public void RefreshMenu()
    {
        ...
        {
            RebuildCategoryButtons(definitions);
            CreateCatalogSlots(definitions);
            return;
        }
    ...
    private void CreateDefinitionSlot(BuildDefinitionSO definition)
    {
        ...
        bool resourceUnlocked = StockpileManager.Instance != null
            && StockpileManager.Instance.HasUnlockedResource(definition.RequiredResource);
        bool isUnlocked = recipeUnlocked && resourceUnlocked;
        ...
        view.Bind(definition, isUnlocked,
            GetLockReason(definition.RequiredResource, recipeUnlocked, resourceUnlocked),
            tooltip,
    ...
    private static string GetLockReason(ResourceType resource, bool recipeUnlocked, bool resourceUnlocked)
    {
        if (!recipeUnlocked) return "Receita bloqueada";
        if (!resourceUnlocked) return "Requer recurso desbloqueado: " + resource;
        return string.Empty;
    }
}
