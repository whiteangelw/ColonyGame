...
public class StructureManager : Singleton<StructureManager>
{
    ...
    public ConfiguredStructure GetConfiguredStructureAt(Vector2Int position)
    {
        if (GridManager.Instance != null)
        {
            UnityEngine.Object occupant = GridManager.Instance.GetOccupantAt(
                position,
                GridLayer.Structure);
            if (occupant is ConfiguredStructure configuredOccupant)
            {
                return configuredOccupant;
            }
        }

        activeConfiguredStructures.TryGetValue(
            position,
            out ConfiguredStructure configured);
        return configured;
    }
