...
public class DuplicantMovement : MonoBehaviour
{
    ...
    public MovementType DeduceMovementType(Vector2Int from, Vector2Int to)
    {
        ...
        bool isVerticalLadderTransition = GridManager.Instance != null
            && dx == 0
            && dy != 0
            && (GridManager.Instance.IsLadder(from.x, from.y)
                || GridManager.Instance.IsLadder(to.x, to.y));

        if (isVerticalLadderTransition)
