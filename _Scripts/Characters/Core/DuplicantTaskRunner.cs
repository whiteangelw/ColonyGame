...
public class DuplicantTaskRunner : MonoBehaviour
{
    ...
    private ProductionDeliveryExecutor productionDeliveryExecutor;
    private ProductionOperationExecutor productionOperationExecutor;
    ...
    private void Awake()
    {
        ...
        productionDeliveryExecutor = new ProductionDeliveryExecutor(
            controller,
            inventory,
            pathFollower,
            resourceCollector);
        ...
        productionOperationExecutor = new ProductionOperationExecutor(
            controller,
            pathFollower);
    ...
    public IEnumerator ExecuteTaskRoutine(Task task, List<Vector2Int> path)
    {
        ...
        if (task.type == TaskType.HaulResource
            && (task.targetResourceDelivery != null
                || task.targetProductionMachine != null))
        {
            yield return StartCoroutine(ExecuteProductionDeliveryRoutine(task));
            yield break;
        }

        if (task.type == TaskType.OperateMachine
            && task.targetProductionMachine != null)
        {
            yield return StartCoroutine(ExecuteProductionOperationRoutine(task));
            yield break;
        }
    ...
    private IEnumerator ExecuteProductionDeliveryRoutine(Task task)
    {
        yield return productionDeliveryExecutor.Execute(task);

        switch (productionDeliveryExecutor.Result)
        {
            case ProductionDeliveryResult.Completed:
                IResourceDeliveryTarget deliveredTarget =
                    task.targetResourceDelivery ?? task.targetProductionMachine;
                TaskManager.Instance?.RemoveTask(task);
                deliveredTarget?.ReevaluateTasks();
                FinishCurrentTask();
                yield break;

            case ProductionDeliveryResult.ResourceUnavailable:
            case ProductionDeliveryResult.NothingReserved:
                TaskManager.Instance?.ReleaseTask(task);
                FinishCurrentTask();
                controller.Brain?.RequestImmediateTaskSearch();
                yield break;

            case ProductionDeliveryResult.DestinationUnreachable:
                CancelTask(
                    task,
                    TaskFailureReason.TargetUnreachable,
                    "A máquina ficou inalcançável durante a entrega.");
                yield break;
    ...
    private IEnumerator ExecuteProductionOperationRoutine(Task task)
    {
        yield return productionOperationExecutor.Execute(task);

        switch (productionOperationExecutor.Result)
        {
            case ProductionOperationResult.BatchCompleted:
                ProductionMachineBehaviour operatedMachine =
                    task.targetProductionMachine;
                TaskManager.Instance?.RemoveTask(task);
                operatedMachine?.ReevaluateTasks();
                FinishCurrentTask();
                yield break;

            case ProductionOperationResult.DestinationUnreachable:
                CancelTask(
                    task,
                    TaskFailureReason.TargetUnreachable,
                    "Não foi possível alcançar a máquina.");
                yield break;
        ...
        bool preserveCarriedInventory = false)
    {
        ...
        StopAllCoroutines();
        productionDeliveryExecutor?.Cancel();
        activeTask?.HandleWorkInterrupted();
        ReleaseAllReservations();
