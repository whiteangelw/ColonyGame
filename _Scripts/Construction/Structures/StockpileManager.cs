...
public class StockpileManager : MonoBehaviour
{
    ...
    public void RefreshReachableResources()
    {
        ...
        foreach (ResourceType type in AllResourceTypes)
        {
            ...
            // A reserva representa um recurso que pode estar em trânsito.
            // Não limite a reserva ao total observado neste instante: durante
            // uma retirada do baú existe uma janela curta em que o recurso já
            // saiu do armazenamento, mas ainda não entrou no inventário.
        ...
        int amount)
    {
        ...
        // A entrega consome o recurso. Atualize o total em cache
        // imediatamente para impedir que outra tarefa reserve a mesma
        // quantidade antes do próximo recálculo físico do estoque.
        int previousAmount = GetAmount(type);
        int updatedAmount = Mathf.Max(0, previousAmount - amount);
        resources[type] = updatedAmount;
