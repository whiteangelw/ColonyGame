...
public class DuplicantVitals : MonoBehaviour
{
    ...
    public bool IsBedResting { get; private set; }
    ...
    public void ApplyTick(LifeCycleSettingsSO settings, float elapsedSeconds)
    {
        ...
        if (!IsEmergencyResting && !IsBedResting)
        {
    ...
    public void RestoreEnergy(float amount)
    {
        ...
        currentEnergy = Mathf.Min(
            MaximumEnergy,
            currentEnergy + Mathf.Max(0f, amount) * multiplier);
        emergencyRequested = false;
        EvaluateState();
        OnVitalsChanged?.Invoke(this);
    }
    ...
    public void EndBedRest()
    {
        if (!IsBedResting) return;
        IsBedResting = false;
        emergencyRequested = false;
        EvaluateState();
        OnVitalsChanged?.Invoke(this);
